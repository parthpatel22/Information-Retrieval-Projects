...
import cs547
MY_NAME = "Parth Patel"
MY_ANUM  = 661591670 # put your UID here
...
class BetterIndex(object):
    ...
    def wildcard_search_or(self, text):
        ...
        for i in text.split(" "):
            if self.WILDCARD not in i:
                t1 = self.tokenize(i)
                k = self._rotate(t1[0])
            else:
                t1 = self.tokenize(i,True)
                k = self._rotate((t1[0]))
            p = (crawl_tree(self._bt.root,k))
            
            temp = set.union(temp,p)
        temp = map(str,list(temp))
...
def main(args):
    ...
    for term in ('hel*o', 'aggies', 'agg*', 'mike sherm*', 'dot cat','hel*o', 'Sherm*', 'his*', 'Mike sherm*', 'his* sherm*'):
